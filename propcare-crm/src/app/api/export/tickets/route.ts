...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = supabase
      .from("tickets")
      .select(`
        code, title, status, priority, category, project, source,
        contact_status, created_at, due_date, resolved_at, sla_hours,
        client:clients(name, phone),
        unit:client_units(unit_number, project),
    ...
    const rows = (data ?? []).map((t: Record<string, unknown>) => {
      ...
      const unit       = t.unit        as { unit_number?: string; project?: string } | null;
    ...
    const header = [
      "Code","Title","Status","Priority","Category","Project","Source","Contact Status",
      "Client","Phone","Unit Number","Unit Project","Assigned To","SLA Hours",
      "Created At","Due Date","Resolved At",
    ].join(",");
