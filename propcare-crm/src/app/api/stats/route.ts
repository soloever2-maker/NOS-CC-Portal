...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      resolvedBase().eq("status", "RESOLVED").not("resolved_at", "is", null),
      resolvedBase().eq("status", "CLOSED").not("resolved_at", "is", null),
    ...
    const { data: byCat } = await catQuery;
    const categoryCounts = (byCat ?? []).reduce<Record<string, number>>((acc, t) => {
    ...
    const { data: bySrc } = await srcQuery;
    const sourceCounts = (bySrc ?? []).reduce<Record<string, number>>((acc, t) => {
      const s = t.source ?? "unknown";
      acc[s] = (acc[s] ?? 0) + 1; return acc;
    }, {});

    // ── Avg Resolution Time ─────────────────────────────────────────────
    let resQuery = supabase.from("tickets")
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        agentPerformance,
        sla: {
          within: slaWithin, atRisk: slaAtRisk, overdue: slaOverdue,
          complianceRate: slaComplianceRate,
          resolvedWithinSLA, resolvedBreached,
        },
      },
    });
  } catch (err) {
