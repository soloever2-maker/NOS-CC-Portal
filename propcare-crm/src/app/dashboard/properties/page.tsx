...
import { Plus, Search, Building2, Bed, Bath, Maximize2, MoreHorizontal, Eye, Trash2, MapPin, AlertCircle } from "lucide-react";
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
            {properties.map(prop => (
              <div key={prop.id} className="crm-card overflow-hidden group transition-all hover:border-[var(--border-strong)] hover:shadow-[0_0_24px_var(--gold-glow)]">
                <div className="h-36 relative flex items-center justify-center" style={{ background: "linear-gradient(135deg, var(--black-700), var(--black-600))" }}>
                  <Building2 className="w-10 h-10 opacity-20" style={{ color: TYPE_COLOR[prop.type] }} />
                  <div className="absolute top-3 left-3"><Badge variant={STATUS_BADGE[prop.status]}>{PROPERTY_STATUS_LABELS[prop.status]}</Badge></div>
                  <div className="absolute top-3 right-3">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="w-7 h-7 rounded-[6px] flex items-center justify-center opacity-0 group-hover:opacity-100" style={{ background: "var(--black-800)", color: "var(--text-muted)" }}><MoreHorizontal className="w-3.5 h-3.5" /></button>
                      </DropdownMenuTrigger>
                ...
                <div className="p-4">
                  ...
                  {(prop.project || prop.city) && (
                    <div className="flex items-center gap-1 mb-3"><MapPin className="w-3 h-3" style={{ color: "var(--text-muted)" }} /><p className="text-xs" style={{ color: "var(--text-muted)" }}>{[prop.project, prop.city].filter(Boolean).join(" · ")}</p></div>
                  )}
              ...
              <tbody>
                {properties.map(prop => (
                  <tr key={prop.id} className="group">
                    <td>
                      <Link href={`/dashboard/properties/${prop.id}`} className="text-sm font-medium hover:text-[var(--gold-400)]" style={{ color: "var(--text-primary)" }}>{prop.name}</Link>
                      <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>{prop.code}</p>
                    </td>
                    <td><span className="text-sm" style={{ color: TYPE_COLOR[prop.type] }}>{PROPERTY_TYPE_LABELS[prop.type]}</span></td>
                    ...
                    <td>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <button className="w-7 h-7 rounded-[6px] flex items-center justify-center opacity-0 group-hover:opacity-100" style={{ color: "var(--text-muted)" }}><MoreHorizontal className="w-4 h-4" /></button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild><Link href={`/dashboard/properties/${prop.id}`}><Eye className="w-4 h-4" />View</Link></DropdownMenuItem>
                          <DropdownMenuSeparator />
