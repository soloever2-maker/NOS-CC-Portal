...
import { TrendingUp, CheckCircle, Clock, Users, Download, ShieldCheck, ShieldAlert, ShieldX, Timer, PhoneCall } from "lucide-react";
...
const STATUS_COLORS: Record<string, string> = {
  OPEN: "#3b82f6", IN_PROGRESS: "#f59e0b", PENDING_CLIENT: "#8b5cf6", RESOLVED: "#22c55e", CLOSED: "#606060",
};
...
function getRange(days: number): { from: string; to: string } | Record<string, never> {
  if (days === 0) return {};
  const to   = new Date();
...
// ── Loading Spinner ───────────────────────────────────
const Spinner = () => <div className="flex justify-center py-8"><div className="w-5 h-5 border-2 border-t-[var(--gold-500)] rounded-full animate-spin" /></div>;
...
export default function ReportsPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    if ("from" in range) { params.set("from", range.from); params.set("to", range.to); }
  ...
  return (
    <div className="flex flex-col min-h-screen animate-fade-in">
      <Topbar title="Reports" subtitle="Live analytics"
        actions={
      ...
      <div className="flex-1 p-5 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* Date range */}
          <div className="flex gap-1 p-1 rounded-[10px]" style={{ background: "var(--black-800)", border: "1px solid var(--border)" }}>
            {DATE_RANGES.map(r => (
              <button key={r.label} onClick={() => setRangeDays(r.days)}
                className="px-3 py-1.5 rounded-[7px] text-xs font-semibold transition-all"
                style={{
                  background: rangeDays === r.days ? "var(--gold-glow)" : "transparent",
                  color: rangeDays === r.days ? "var(--gold-400)" : "var(--text-muted)",
                  border: rangeDays === r.days ? "1px solid var(--border-strong)" : "1px solid transparent",
                }}>
                {r.label}
              </button>
            ))}
          ...
          <select
            ...
            className="crm-input h-8 text-xs px-3"
            style={{ minWidth: 160 }}
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            { label: "Resolved Today", value: stats?.resolvedToday ?? 0, icon: CheckCircle, color: "var(--success)",     fmt: (v: number) => String(v) },
            { label: "Clients",      value: stats?.totalClients ?? 0,    icon: Users,       color: "var(--info)",        fmt: (v: number) => String(v) },
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
            {[
              ...
              { label: "Breached",   value: sla?.overdue ?? 0, icon: ShieldX,     color: "var(--danger)",  bg: "rgba(239,68,68,0.08)"  },
            ].map(s => (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-xs font-semibold tracking-wider" style={{ color: "var(--text-muted)" }}>ACTIVE — SLA STATUS</CardTitle></CardHeader>
              <CardContent>
                {loading ? <Spinner /> : slaChartData.length === 0
                  ...
                  : <ResponsiveContainer width="100%" height={200}>
                      <PieChart>
                        <Pie data={slaChartData} cx="50%" cy="50%" outerRadius={75} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={{ stroke: "var(--text-muted)", strokeWidth: 0.5 }}>
                          {slaChartData.map((e, i) => <Cell key={i} fill={e.fill} />)}
                        </Pie>
            ...
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-xs font-semibold tracking-wider" style={{ color: "var(--text-muted)" }}>RESOLVED — SLA COMPLIANCE (THIS MONTH)</CardTitle></CardHeader>
              <CardContent>
                {loading ? <Spinner /> : slaResolutionData.length === 0
                  ? <p className="text-center text-sm py-8" style={{ color: "var(--text-muted)" }}>No resolved tickets with SLA</p>
                  : <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={slaResolutionData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barSize={48}>
                        ...
                        <Bar dataKey="value" name="Tickets" radius={[4,4,0,0]}>
                          {slaResolutionData.map((e, i) => <Cell key={i} fill={e.fill} opacity={0.85} />)}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>}
              </CardContent>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs font-semibold tracking-wider" style={{ color: "var(--text-muted)" }}>BY CATEGORY</CardTitle></CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs font-semibold tracking-wider" style={{ color: "var(--text-muted)" }}>BY STATUS</CardTitle></CardHeader>
            <CardContent>
              {loading ? <Spinner /> : statusData.length === 0
                ? <p className="text-center text-sm py-8" style={{ color: "var(--text-muted)" }}>No data</p>
                : <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie data={statusData} cx="50%" cy="50%" outerRadius={75} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={{ stroke: "var(--text-muted)", strokeWidth: 0.5 }}>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-xs font-semibold tracking-wider flex items-center gap-2" style={{ color: "var(--text-muted)" }}>
              <Users className="w-3.5 h-3.5" style={{ color: "var(--gold-500)" }} /> AGENT PERFORMANCE
            </CardTitle>
          </CardHeader>
                    ...
                    {stats.agentPerformance.map((a, i) => {
                      ...
                      return (
                        <tr key={i}>
                          ...
                          <td>
                            <div className="flex items-center gap-2">
                              <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--black-600)", maxWidth: 80 }}>
                                <div className="h-full rounded-full" style={{ width: `${rate}%`, background: rate >= 70 ? "var(--success)" : rate >= 40 ? "var(--warning)" : "var(--danger)" }} />
                              </div>
                              <span className="text-xs font-semibold" style={{ color: rate >= 70 ? "var(--success)" : rate >= 40 ? "var(--warning)" : "var(--danger)" }}>{rate}%</span>
                            </div>
                          </td>
                        </tr>
                      );
