...
export default function TicketsPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen animate-fade-in">
      <Topbar title="Tickets" subtitle={`${tickets.length} ticket${tickets.length !== 1 ? "s" : ""}`}
      ...
      <div className="flex-1 p-6 space-y-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button variant="outline" size="sm" className="gap-1.5"><Filter className="w-3.5 h-3.5" /> Filters<ChevronDown className="w-3 h-3" /></Button>
        </div>

        <div className="flex gap-1 p-1 rounded-[10px] w-fit" style={{ background: "var(--black-800)", border: "0.5px solid var(--border)" }}>
          {STATUSES.map((s) => (
            <button key={s} onClick={() => setActiveStatus(s)} className="px-3 py-1.5 rounded-[7px] text-xs font-medium transition-all whitespace-nowrap flex items-center gap-1.5"
              ...
              <span className="px-1.5 py-0 rounded-full text-[10px] font-bold" style={{ background: activeStatus === s ? "var(--gold-500)" : "var(--black-600)", color: activeStatus === s ? "var(--black-950)" : "var(--text-muted)" }}>
                {counts[s] ?? 0}
              </span>
            </button>
          ))}
        ...
        <div className="rounded-[12px] overflow-hidden" style={{ border: "0.5px solid var(--border)", background: "var(--black-800)" }}>
          <table className="crm-table">
            <thead>
              <tr><th>Ticket</th><th>Client / Property</th><th>Status</th><th>Priority</th><th>Category</th><th>Assigned To</th><th>Created</th><th></th></tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="text-center py-12" style={{ color: "var(--text-muted)" }}>
                  <div className="w-6 h-6 border-2 border-t-[var(--gold-500)] rounded-full animate-spin mx-auto" />
                </td></tr>
              ) : tickets.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-12" style={{ color: "var(--text-muted)" }}>
                  <AlertCircle className="w-8 h-8 mx-auto mb-2 opacity-40" /><p className="text-sm">No tickets found</p>
                </td></tr>
              ) : tickets.map((ticket) => {
                ...
                return (
                  <tr key={ticket.id} className="group">
                    ...
                    <td>
                      <div>
                        <div className="flex items-center gap-1.5"><User className="w-3 h-3 shrink-0" style={{ color: "var(--text-muted)" }} /><span className="text-sm" style={{ color: "var(--text-secondary)" }}>{ticket.client?.name ?? "—"}</span></div>
                        {ticket.property && <div className="flex items-center gap-1.5 mt-0.5"><Building2 className="w-3 h-3 shrink-0" style={{ color: "var(--text-muted)" }} /><span className="text-[11px] line-clamp-1" style={{ color: "var(--text-muted)" }}>{ticket.property.name}</span></div>}
                      </div>
                    </td>
                    ...
                    <td><span className="text-xs" style={{ color: "var(--text-secondary)" }}>{TICKET_CATEGORY_LABELS[ticket.category]}</span></td>
                    <td>
                      {ticket.assigned_to ? (
                        <div className="flex items-center gap-1.5">
                          <div className="w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-bold" style={{ background: "var(--gold-glow)", color: "var(--gold-500)", border: "1px solid var(--border)" }}>
                            {ticket.assigned_to.name.split(" ").map(n => n[0]).join("")}
                          </div>
