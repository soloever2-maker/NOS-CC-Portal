...
import {
  Plus, Search, Clock, User, AlertCircle, Download,
...
import { TICKET_STATUS_LABELS, TICKET_PRIORITY_LABELS, TICKET_CATEGORY_LABELS, CONTACT_STATUS_LABELS, CONTACT_STATUS_COLORS, type TicketStatus, type TicketPriority, type TicketCategory, type ContactStatus } from "@/types";
...
interface Ticket {
  ...
  source?: string | null; contact_status?: ContactStatus | null; assigned_to: { name: string } | null;
  ...
  return (
    <div className="flex flex-col min-h-screen animate-fade-in">
      <Topbar
        ...
        actions={<div className="flex gap-2"><Button size="sm" variant="outline" onClick={async () => { const r = await fetch("/api/export/tickets"); const b = await r.blob(); const u = URL.createObjectURL(b); const a = document.createElement("a"); a.href=u; a.download=`tickets-${new Date().toISOString().split("T")[0]}.csv`; a.click(); URL.revokeObjectURL(u); }}><Download className="w-3.5 h-3.5" /></Button><Button size="sm" asChild><Link href="/dashboard/tickets/new"><Plus className="w-3.5 h-3.5" /> New Ticket</Link></Button></div>}
      />
      ...
      <div className="flex-1 p-5 space-y-4">
        ...
        <div className="rounded-[12px] overflow-hidden" style={{ border: "0.5px solid var(--border)", background: "var(--black-800)" }}>
          <table className={`crm-table${compact ? " compact" : ""}`}>
            <thead>
              <tr><th>Ticket</th><th>Project</th><th>Client</th><th>Status</th><th>Contact</th><th>Priority</th><th>SLA</th><th>Assigned To</th><th>Created</th><th></th></tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={ticket.id} className="group">
                    ...
                    <td>{ticket.contact_status ? (() => { const cs = CONTACT_STATUS_COLORS[ticket.contact_status]; return <span className="inline-flex items-center px-1.5 py-0.5 rounded-full font-semibold whitespace-nowrap" style={{ fontSize:10, background:cs.bg, color:cs.color, border:`1px solid ${cs.color}44` }}>{CONTACT_STATUS_LABELS[ticket.contact_status]}</span>; })() : <span style={{color:"var(--text-muted)"}}>—</span>}</td>
